 ...
 */
function createData(version: number, errorCorrectionLevel: ErrorCorrectionLevel, segments: ByteData[]): Uint8Array {
  // Prepare data buffer
  ...
  for (const data of segments) {
    ...
    buffer.put(
      data.getLength(),
      getCharCountIndicator(data.mode, version),
    )
